...
def fix_test_run_pipeline():
    ...
    def test_run_pipeline(mock_print, mock_preprocess, mock_collect_data):
        ...
        with mock.patch('datetime.datetime') as mock_datetime:
            fixed_time = datetime(2025, 5, 11, 12, 0, 0)
            ...
            expected_start_message = f"[{fixed_time}] Starting Weather Data Pipeline..."
            expected_collect_data_message = f"[{fixed_time}] Running data collection..."
            expected_preprocess_message = f"[{fixed_time}] Running preprocessing..."
            expected_end_message = f"[{fixed_time}] Pipeline completed successfully!"
