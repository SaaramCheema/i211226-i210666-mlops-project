...
@mock.patch.object(datetime, "now", return_value=datetime(2025, 5, 11, 12, 0, 0))
def test_collect_weather_data(mock_datetime, mock_csv_writer, mock_requests_get):
    ...
    # Expected data written to the CSV
    weather_data = {
        "datetime": datetime(2025, 5, 11, 12, 0, 0).isoformat(),
        "temperature": 25,
        "humidity": 60,
        "wind_speed": 10,
        "condition": "Clear"
    ...
    # Ensure that the header is written (if the CSV is empty)
